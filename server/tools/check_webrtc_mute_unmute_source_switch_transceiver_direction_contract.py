...
def _check_doc(errors: list[str]) -> None:
    if not DOC_25_3_PATH.exists():
        _fail(errors, f"Missing Step 25.3 baseline doc: {DOC_25_3_PATH}")
        return

    text = DOC_25_3_PATH.read_text(encoding="utf-8")
    for phrase in REQUIRED_DOC_25_3_PHRASES:
        if phrase not in text:
            _fail(errors, f"Missing phrase in Step 25.3 baseline doc: {phrase}")

    if not DOC_25_2_PATH.exists():
        _fail(errors, f"Missing Step 25.2 dependency doc: {DOC_25_2_PATH}")


def _check_linked_paths(errors: list[str]) -> None:
    for path in REQUIRED_LINKED_PATHS:
        if not path.exists():
            _fail(errors, f"Missing linked Step 25.3 baseline path: {path}")
...
def _check_mapping_contract(errors: list[str]) -> None:
    ...
    try:
        from app.webrtc_mute_unmute_source_switch_transceiver_direction_contract import (
            validate_webrtc_mute_unmute_source_switch_transceiver_direction_contract,
        )
    except Exception as exc:  # pragma: no cover - defensive import check
        _fail(errors, f"Failed to import Step 25.3 baseline module: {exc}")
        return

    errors.extend(validate_webrtc_mute_unmute_source_switch_transceiver_direction_contract())
...
def main() -> int:
    ...
    if errors:
        print("webrtc-mute-unmute-source-switch-transceiver-direction-contract-check: FAILED")
        for issue in errors:
            print(f"- {issue}")
        return 1

    print("webrtc-mute-unmute-source-switch-transceiver-direction-contract-check: OK")
    print(f"- checked Step 25.3 doc: {DOC_25_3_PATH}")
    return 0
