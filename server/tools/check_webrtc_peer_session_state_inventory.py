...
def _check_doc(errors: list[str]) -> None:
    if not DOC_24_2_PATH.exists():
        _fail(errors, f"Missing Step 24.2 baseline doc: {DOC_24_2_PATH}")
        return

    text = DOC_24_2_PATH.read_text(encoding="utf-8")
    for phrase in REQUIRED_DOC_24_2_PHRASES:
        if phrase not in text:
            _fail(errors, f"Missing phrase in Step 24.2 baseline doc: {phrase}")

    if not DOC_24_1_PATH.exists():
        _fail(errors, f"Missing Step 24.1 dependency doc: {DOC_24_1_PATH}")


def _check_linked_paths(errors: list[str]) -> None:
    for path in REQUIRED_LINKED_PATHS:
        if not path.exists():
            _fail(errors, f"Missing linked lifecycle inventory path: {path}")
...
def _check_mapping_contract(errors: list[str]) -> None:
    ...
    try:
        from app.webrtc_peer_session_state_inventory import (
            validate_webrtc_peer_session_state_inventory,
        )
    except Exception as exc:  # pragma: no cover - defensive import check
        _fail(errors, f"Failed to import Step 24.2 baseline module: {exc}")
        return

    errors.extend(validate_webrtc_peer_session_state_inventory())
...
def main() -> int:
    ...
    if errors:
        print("webrtc-peer-session-state-inventory-check: FAILED")
        for issue in errors:
            print(f"- {issue}")
        return 1

    print("webrtc-peer-session-state-inventory-check: OK")
    print(f"- checked Step 24.2 doc: {DOC_24_2_PATH}")
    return 0
