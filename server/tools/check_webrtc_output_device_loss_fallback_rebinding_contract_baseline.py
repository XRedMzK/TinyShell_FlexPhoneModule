...
def _check_doc(errors: list[str]) -> None:
    if not DOC_27_4_PATH.exists():
        _fail(errors, f"Missing Step 27.4 baseline doc: {DOC_27_4_PATH}")
        return

    text = DOC_27_4_PATH.read_text(encoding="utf-8")
    for phrase in REQUIRED_DOC_27_4_PHRASES:
        if phrase not in text:
            _fail(errors, f"Missing phrase in Step 27.4 baseline doc: {phrase}")

    if not DOC_27_3_PATH.exists():
        _fail(errors, f"Missing Step 27.3 dependency doc: {DOC_27_3_PATH}")


def _check_linked_paths(errors: list[str]) -> None:
    for path in REQUIRED_LINKED_PATHS:
        if not path.exists():
            _fail(errors, f"Missing linked Step 27.4 baseline path: {path}")
...
def _check_mapping_contract(errors: list[str]) -> None:
    ...
    try:
        from app.webrtc_output_device_loss_fallback_rebinding_contract_baseline import (
            validate_webrtc_output_device_loss_fallback_rebinding_contract_baseline,
        )
    except Exception as exc:  # pragma: no cover - defensive import check
        _fail(errors, f"Failed to import Step 27.4 baseline module: {exc}")
        return

    errors.extend(validate_webrtc_output_device_loss_fallback_rebinding_contract_baseline())
...
def main() -> int:
    ...
    if errors:
        print("webrtc-output-device-loss-fallback-rebinding-contract-check: FAILED")
        for issue in errors:
            print(f"- {issue}")
        return 1

    print("webrtc-output-device-loss-fallback-rebinding-contract-check: OK")
    print(f"- checked Step 27.4 doc: {DOC_27_4_PATH}")
    return 0
