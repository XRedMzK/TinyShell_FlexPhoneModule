...
from run_webrtc_remote_video_resilience_sandbox_smoke import (
...
EVIDENCE_PATH = (
    ...
    / "docs"
    / "evidence"
    / "step29-remote-video-resilience-sandbox-smoke-evidence.json"
)
CI_SUCCESS_MARKER = "WebRTC remote video resilience CI gate: OK"
CI_UNSUPPORTED_MARKER = "WebRTC remote video resilience CI gate: UNSUPPORTED"
...
def _validate_manifest_structure(payload: dict[str, Any]) -> list[str]:
    ...
    for entry in scenarios:
        ...
        for field in CI_REMOTE_VIDEO_EVIDENCE_SCHEMA_REQUIRED_FIELDS:
        ...
        if result not in CI_REMOTE_VIDEO_OUTCOME_MODEL:
            errors.append(
                f"{scenario_id}: result must be one of {CI_REMOTE_VIDEO_OUTCOME_MODEL}, got {result!r}"
            )
    ...
    expected_ids = set(REMOTE_VIDEO_SMOKE_SCENARIO_IDS)
    if set(scenario_map.keys()) != expected_ids:
        errors.append(f"scenario ID set mismatch, expected {sorted(expected_ids)}")

    for scenario_id, required_markers in CI_REMOTE_VIDEO_REQUIRED_INVARIANTS.items():
...
def run_ci_gate() -> int:
    if not EVIDENCE_PATH.exists():
        print(f"WebRTC remote video resilience CI gate: FAIL - missing evidence manifest: {EVIDENCE_PATH}")
        return 1

    try:
        payload = json.loads(EVIDENCE_PATH.read_text(encoding="utf-8"))
    except Exception as exc:
        print(f"WebRTC remote video resilience CI gate: FAIL - invalid JSON evidence: {exc}")
        return 1

    if not isinstance(payload, dict):
        print("WebRTC remote video resilience CI gate: FAIL - evidence root must be object")
        return 1

    errors = _validate_manifest_structure(payload)
    if errors:
        print("WebRTC remote video resilience CI gate: FAIL")
        for issue in errors:
            print(f"- {issue}")
        return 1
    ...
    if has_fail:
        print("WebRTC remote video resilience CI gate: FAIL - smoke evidence contains FAIL scenario")
        return 1
...
def main() -> int:
    try:
        ...
        print(f"WebRTC remote video resilience CI gate: FAIL (unexpected) - {exc!r}")
        return 1
